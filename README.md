...
网站主身份为产品体验设计师，差异化方向为复杂业务与 AI 工作流、原型和工具实践。首页使用文字作品索引，媒体证据位于案例页。
...
- `/work/edgecase-planner/` 展示从真实设计评审中整理 AI 检查规则的工具实践。
...
- `npm run test:e2e` 检查整站四档宽度、键盘访问、减弱动效和深链，首页与 GIS 折叠专项扩展到六档宽度。
...
用户于 2026-09-15 确认网站已公开发布。2026-09-16 的投递前修改尚未提交或发布。当前仓库没有记录线上地址或自动部署命令，再次发布时需先确认实际托管平台和地址。
...
public/projects/      四个案例的公开媒体与原型复制品
tests/                内容、资源、行为和视觉检查
...
- 案例图片、视频和原型预览会阻止右键菜单与拖拽，普通页面区域保持原有操作。这项处理不是加密，公开资源仍可通过开发者工具读取或被截图。
...
文档阅读顺序、有效说明与历史材料范围见 [文档索引](docs/README.md)。
